...
$stmt->bind_param("ssssss", $id, $nom_dmd, $desc_dmd, $loca, $date_fin, $id_client);
...
foreach ($data['produit_contenu'] as $produit) {
    ...
    $stmt2->bind_param("ssss", $produit['id_produit'], $id, $produit['nb_produit'], $produit['poids_piece_produit']);
    if (!$stmt2->execute()) {
        $all_inserted = false;
